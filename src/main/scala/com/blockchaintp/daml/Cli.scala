...
object Cli {
  ...
  private val pemConfig = (path: String, config: DamlOnQldbConfig) =>
    config.copy(
  ...
  private val crtConfig = (path: String, config: DamlOnQldbConfig) =>
    config.copy(
      tlsConfig = config.tlsConfig.fold(
  ...
  private val cacrtConfig = (path: String, config: DamlOnQldbConfig) =>
    config.copy(
      tlsConfig = config.tlsConfig.fold(
  ...
  private def cmdArgParser(
    ...
    new scopt.OptionParser[DamlOnQldbConfig](binaryName) {
      head(
        description
      )
      opt[Int]("port")
      ...
      opt[String]("ledger")
        .optional()
        .text("name of the QLDB ledger to use")
        .action((v, c) => c.copy(ledger = v))
      opt[String]("auth")
        .text("Auth setting. Value wildcard(default) or hmac256=<secret>")
        .optional()
  ...
  def parse(
      ...
      binaryName: String,
      description: String,
  ): Option[DamlOnQldbConfig] =
    cmdArgParser(binaryName, description)
      .parse(args, DamlOnQldbConfig.default)
