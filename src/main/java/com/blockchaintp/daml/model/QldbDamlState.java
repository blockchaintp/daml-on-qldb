package com.blockchaintp.daml.model;

import com.daml.ledger.participant.state.kvutils.DamlKvutils.DamlStateKey;
import com.daml.ledger.participant.state.kvutils.DamlKvutils.DamlStateValue;
import com.daml.ledger.participant.state.kvutils.KeyValueCommitting;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.google.protobuf.ByteString;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

public final class QldbDamlState extends QldbDamlObject {

  private static final Logger LOG = LoggerFactory.getLogger(QldbDamlState.class);
  public static final String TABLE_NAME = "kv_daml_state";

  public QldbDamlState(@JsonProperty("id") final String newId, @JsonProperty("s3Key") final String newS3key,
      final byte[] data) {
    super(newId, newS3key, data);
  }

  public QldbDamlState(@JsonProperty("id") final String newId) {
    super(newId);
  }

  public QldbDamlState(@JsonProperty("id") final String id, final byte[] data) {
    super(id, data);
  }

  public static QldbDamlState create(final DamlStateKey pbEntryId, final DamlStateValue pbEntry) {
    final String packedId = KeyValueCommitting.packDamlStateKey(pbEntryId).toStringUtf8();
    final byte[] data = KeyValueCommitting.packDamlStateValue(pbEntry).toByteArray();
    return new QldbDamlState(packedId, data);
  }

  public DamlStateKey damlStateKey() {
    return KeyValueCommitting.unpackDamlStateKey(ByteString.copyFromUtf8(getId()));
  }

  public DamlStateValue damlStateValue() {
    return KeyValueCommitting.unpackDamlStateValue(ByteString.copyFrom(s3Data()));
  }

  @Override
  public String tableName() {
    return QldbDamlState.TABLE_NAME;
  }
}
