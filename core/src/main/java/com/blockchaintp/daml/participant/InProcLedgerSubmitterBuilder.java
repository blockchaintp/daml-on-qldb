...
public final class InProcLedgerSubmitterBuilder<I extends Identifier, A extends LedgerAddress> {
   ...
   * @return A configured builder.
   */
  public InProcLedgerSubmitterBuilder<I, A> withContext(final ExecutionContextExecutor theContext) {
    context = theContext;
   ...
   * @return A configured t builder.
   */
  public InProcLedgerSubmitterBuilder<I, A> withRetryAttempts(final int theRetryAttempts) {
  ...
  public LedgerSubmitter<I, A> build() {
    var inproc = new InProcLedgerSubmitter<I, A>(engine, metrics, txLog, stateStore, dispatcher, context);
    return new LedgerSubmitterBulkhead<>(inproc, maxConcurrent, slowCallDuration, retryAttempts);
  }
