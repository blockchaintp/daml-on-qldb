/*
 * Copyright 2008-2022 Blockchain Technology Partners
 *
...
public abstract class CodedInputStream {
  ...
  /**
   * Create a new CodedInputStream wrapping the given ByteBuffer. The data starting from the ByteBuffer's current
   * position to its limit will be read. The returned CodedInputStream may or may not share the underlying data in the
   * ByteBuffer, therefore the ByteBuffer cannot be changed while the CodedInputStream is in use. Note that the
   * ByteBuffer's position won't be changed by this function. Concurrent calls with the same ByteBuffer object are safe
   * if no other thread is trying to alter the ByteBuffer's status.
   */
  ...
  private CodedInputStream() {
    if (null != System.getenv("RECURSION_LIMIT")) {
      recursionLimit = Integer.parseInt(System.getenv("RECURSION_LIMIT"));
    }
  }
   ...
   * @return {@code false} if the tag is an endgroup tag, in which case nothing is skipped. Otherwise, returns
   *         {@code true}.
   * @deprecated use {@code UnknownFieldSet} or {@code UnknownFieldSetLite} to skip to an output stream.
   */
  @Deprecated
  ...
  /**
   * Reads an entire message and writes it to output in wire format. This will read either until EOF or until an
   * endgroup tag, whichever comes first.
   */
  ...
  /**
   * Read a {@code string} field value from the stream. If the stream contains malformed UTF-8, replace the offending
   * bytes with the standard UTF-8 replacement character.
   */
  public abstract String readString() throws IOException;

  /**
   * Read a {@code string} field value from the stream. If the stream contains malformed UTF-8, throw exception
  ...
  /**
   * Enables {@link ByteString} aliasing of the underlying buffer, trading off on buffer pinning for data copies. Only
   * valid for buffer-backed streams.
   */
   ...
   * {@code CodedInputStream} limits how deeply messages may be nested. The default limit is 100.
   *
   ...
   * overflows, {@code CodedInputStream} limits how large a message may be. The default limit is
   * {@code Integer.MAX_INT}. You should set this limit as small as you can without harming your app's functionality.
   * Note that size limits only apply when reading from an {@code
   * InputStream}, not when constructed around a raw byte array.
   *
   * <p>
   * If you want to read several messages from a single CodedInputStream, you could call {@link #resetSizeCounter()}
  ...
  /**
   * Sets this {@code CodedInputStream} to discard unknown fields. Only applies to full runtime messages; lite messages
   * will always preserve unknowns.
   *
   * <p>
   * Note calling this function alone will have NO immediate effect on the underlying input data. The unknown fields
   * will be discarded during parsing. This affects both Proto2 and Proto3 full runtime.
   */
  ...
  /**
   * Resets the current size counter to zero (see {@link #setSizeLimit(int)}). Only valid for {@link InputStream}-backed
   * streams.
   */
   ...
   *
   * <p>
   * Note that {@code pushLimit()} does NOT affect how many bytes the {@code CodedInputStream} reads from an underlying
   * {@code InputStream} when refreshing its buffer. If you need to prevent reading past a certain point in the
   * underlying {@code InputStream} (e.g. because you expect it to contain more data after the end of the message which
   * you need to handle differently) then you must place a wrapper around your {@code InputStream} which limits the
   * amount of data that can be read from it.
   *
   * @return the old limit.
   */
   ...
   * input source has been reached or if the stream has reached a limit created using {@link #pushLimit(int)}. This
   * function may get blocked when using StreamDecoder as it invokes {@link #StreamDecoder.tryRefillBuffer(int)} in this
   * function which will try to read bytes from input.
   */
  ...
  /**
   * Like {@link #readRawVarint32(InputStream)}, but expects that the caller has already read one byte. This allows the
   * caller to determine if EOF has been reached before attempting to read.
   */
  ...
  private static final class ArrayDecoder extends CodedInputStream {
    ...
    @Override
    public <T extends MessageLite> T readMessage(final Parser<T> parser, final ExtensionRegistryLite extensionRegistry)
        throws IOException {
    ...
    public int readRawVarint32() throws IOException {
      ...
      fastpath: {
        ...
        if ((x = buffer[tempPos++]) >= 0) {
          ...
          if (y < 0 && buffer[tempPos++] < 0 && buffer[tempPos++] < 0 && buffer[tempPos++] < 0 && buffer[tempPos++] < 0
              && buffer[tempPos++] < 0) {
    ...
    public int readRawLittleEndian32() throws IOException {
      ...
      return (((buffer[tempPos] & 0xff)) | ((buffer[tempPos + 1] & 0xff) << 8) | ((buffer[tempPos + 2] & 0xff) << 16)
    ...
    public long readRawLittleEndian64() throws IOException {
      ...
      return (((buffer[tempPos] & 0xffL)) | ((buffer[tempPos + 1] & 0xffL) << 8) | ((buffer[tempPos + 2] & 0xffL) << 16)
          | ((buffer[tempPos + 3] & 0xffL) << 24) | ((buffer[tempPos + 4] & 0xffL) << 32)
          | ((buffer[tempPos + 5] & 0xffL) << 40) | ((buffer[tempPos + 6] & 0xffL) << 48)
  ...
  /**
   * A {@link CodedInputStream} implementation that uses a backing direct ByteBuffer as the input. Requires the use of
   * {@code sun.misc.Unsafe} to perform fast reads on the buffer.
   */
  private static final class UnsafeDirectNioDecoder extends CodedInputStream {
    ...
    /**
     * If {@code true}, indicates that the buffer is backing a {@link ByteString} and is therefore considered to be an
    ...
    /**
     * If {@code true}, indicates that calls to read {@link ByteString} or {@code byte[]} <strong>may</strong> return
     * slices of the underlying buffer, rather than copies.
     */
    private boolean enableAliasing;
    ...
    public int readRawLittleEndian32() throws IOException {
      ...
      return (((UnsafeUtil.getByte(tempPos) & 0xff)) | ((UnsafeUtil.getByte(tempPos + 1) & 0xff) << 8)
          | ((UnsafeUtil.getByte(tempPos + 2) & 0xff) << 16) | ((UnsafeUtil.getByte(tempPos + 3) & 0xff) << 24));
    }
    ...
    public long readRawLittleEndian64() throws IOException {
      ...
      return (((UnsafeUtil.getByte(tempPos) & 0xffL)) | ((UnsafeUtil.getByte(tempPos + 1) & 0xffL) << 8)
          | ((UnsafeUtil.getByte(tempPos + 2) & 0xffL) << 16) | ((UnsafeUtil.getByte(tempPos + 3) & 0xffL) << 24)
          | ((UnsafeUtil.getByte(tempPos + 4) & 0xffL) << 32) | ((UnsafeUtil.getByte(tempPos + 5) & 0xffL) << 40)
          | ((UnsafeUtil.getByte(tempPos + 6) & 0xffL) << 48) | ((UnsafeUtil.getByte(tempPos + 7) & 0xffL) << 56));
    }
  ...
  private static final class StreamDecoder extends CodedInputStream {
     ...
     * computed as {@code totalBytesRetired + pos}. This value may be negative if reading started in the middle of the
     * current buffer (e.g. if the constructor that takes a byte array and an offset was used).
     */
    ...
    public ByteBuffer readByteBuffer() throws IOException {
      ...
      // We must copy as the byte array was handed off to the InputStream and a
      // malicious
    ...
    /**
     * Reads more bytes from the input, making at least {@code n} bytes available in the buffer. Caller must ensure that
     * the requested space is not yet available, and that the requested space is less than BUFFER_SIZE.
     *
     * @throws InvalidProtocolBufferException
     *           The end of the stream or the current limit was reached.
     */
    ...
    /**
     * Tries to read more bytes from the input, making at least {@code n} bytes available in the buffer. Caller must
     * ensure that the requested space is not yet available, and that the requested space is less than BUFFER_SIZE.
     *
     * @return {@code true} If the bytes could be made available; {@code false} 1. Current at the end of the stream 2.
     *         The current limit was reached 3. The total size limit was reached
     */
    private boolean tryRefillBuffer(int n) throws IOException {
      ...
      if (bytesRead == 0 || bytesRead < -1 || bytesRead > buffer.length) {
        throw new IllegalStateException(input.getClass() + "#read(byte[]) returned invalid result: " + bytesRead
    ...
    /**
     * Exactly like readRawBytes, but caller must have already checked the fast path: (size <= (bufferSize - pos) &&
     * size > 0)
     *
    ...
    /**
     * Attempts to read the data in one byte array when it's safe to do. Returns null if the size to read is too large
     * and needs to be allocated in smaller chunks for security reasons.
     *
    ...
    /**
     * Like readBytes, but caller must have already checked the fast path: (size <= (bufferSize - pos) && size > 0 ||
     * size == 0)
     */
    ...
    /**
     * Exactly like skipRawBytes, but caller must have already checked the fast path: (size <= (bufferSize - pos) &&
     * size >= 0)
     */
        ...
        try {
          while (totalSkipped < size) {
            ...
            if (skipped < 0 || skipped > toSkip) {
              throw new IllegalStateException(input.getClass() + "#skip returned invalid result: " + skipped
                  + "\nThe InputStream implementation is buggy.");
            } else if (skipped == 0) {
              // The API contract of skip() permits an inputstream to skip zero bytes for any
              // reason
              // it wants. In particular, ByteArrayInputStream will just return zero over and
              // over
              // when it's at the end of its input. In order to actually confirm that we've
              // hit the
  ...
  /**
   * Implementation of {@link CodedInputStream} that uses an {@link Iterable <ByteBuffer>} as the data source. Requires
   * the use of {@code sun.misc.Unsafe} to perform fast reads on the buffer.
   */
  private static final class IterableDirectByteBufferDecoder extends CodedInputStream {
    ...
    /**
     * If {@code true}, indicates that all the buffer are backing a {@link ByteString} and are therefore considered to
     * be an immutable input source.
     */
    ...
    /**
     * If the current ByteBuffer is unsafe-direct based, currentAddress is the start address of this ByteBuffer;
    ...
    public long readRawLittleEndian64() throws IOException {
      ...
      return ((readRawByte() & 0xffL) | ((readRawByte() & 0xffL) << 8) | ((readRawByte() & 0xffL) << 16)
          | ((readRawByte() & 0xffL) << 24) | ((readRawByte() & 0xffL) << 32) | ((readRawByte() & 0xffL) << 40)
          | ((readRawByte() & 0xffL) << 48) | ((readRawByte() & 0xffL) << 56));
    }
     ...
     * bytes} array. If the size is bigger than the number of remaining bytes in the input, then throw
     * {@code truncatedMessage} exception.
     *
     * @param bytes
     * @param offset
     * @param length
    ...
    public void skipRawBytes(final int length) throws IOException {
      if (length >= 0
          && length <= (totalBufferSize - totalBytesRead - currentByteBufferPos + currentByteBufferStartPos)) {
        // We have all the bytes we need already.
        int l = length;
        while (l > 0) {
          if (currentRemaining() == 0) {
